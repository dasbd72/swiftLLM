...
class RequestIdManager:
    ...
    def get_id(self) -> int:
        if not self.available_ids:
            raise RuntimeError(
                "No more available request ids. Please try to increase"
...
class Scheduler:
    ...
    def _get_block_needed(self, request: Request) -> int:
        ...
        """
        return cdiv(
            request.prompt_len + request.get_cur_output_len(),
    ...
    def get_next_batch(
        ...
        if not self.swapped_q:
            ...
            while self.waiting_q:
                ...
                if (
                    len(cur_batch) + 1 <= self.engine_config.max_batch_size
                    and len(self.running_q) + len(cur_batch) + 1
        ...
        newly_swapped_out = []
        while (
            len(self.running_q) > self.engine_config.max_batch_size
        ...
        if newly_swapped_out:
            ...
            while self.swapped_q:
                ...
                if (
                    len(self.running_q) + 1 <= self.engine_config.max_batch_size
    ...
    def on_batch_finish(self, batch: list[Request]):
        ...
        self.running_q = [
            req for req in self.running_q if not req.is_finished()
        ]
