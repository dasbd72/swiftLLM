...
from .structs import RawRequest
from .structs import Request
from .structs import StepOutput
...
class Engine:
    ...
    async def initialize(self):
        ...
        print(
            "[Engine] Number of GPU blocks:"
            f" {num_gpu_blocks} ({num_gpu_blocks*block_size_bytes/GB:.2f} GB)"
        )
        print(
            "[Engine] Number of CPU blocks:"
            f" {num_cpu_blocks} ({num_cpu_blocks*block_size_bytes/GB:.2f} GB)"
        )
    ...
    async def start_all_event_loops(self):
        ...
        assert self.initialized, (
            "Engine not initialized. Please call `initialize()` before starting"
            " the event loop."
        )
