...
def _fwd_rotary_embedding(
    ...
    q += (
        my_token_id * num_q_heads * head_dim
        + my_kv_head * gqa_group_size * head_dim
    )  # [gqa_group_size, head_dim]
    k += (
        my_token_id * num_kv_heads * head_dim + my_kv_head * head_dim
    )  # [head_dim]
    ...
    offs_q0 = (tl.arange(0, gqa_group_size) * head_dim)[:, None] + offs0[
        None, :
    ]
    offs_q1 = (tl.arange(0, gqa_group_size) * head_dim)[:, None] + offs1[
        None, :
    ]
