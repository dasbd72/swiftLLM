...
def _fwd_prefill_attention(
    ...
    offs_my_q = (
        range_my_q[:, None] * (num_q_heads * head_dim)
        + tl.arange(0, head_dim)[None, :]
    )
    my_q = tl.load(
        q + offs_my_q,
        mask=range_my_q[:, None] < my_seq_len,
        cache_modifier=".cg",
    )  # [BLOCK_Q, head_dim]

    k_ptrs = (
        k
        + (tl.arange(0, BLOCK_K))[None, :] * (num_kv_heads * head_dim)
        + tl.arange(0, head_dim)[:, None]
    )
    v_ptrs = (
        v
    ...
    for k_block_start in range(0, my_q_block * BLOCK_Q, BLOCK_K):
        ...
        cur_k = tl.load(
            k_ptrs + k_block_start * (num_kv_heads * head_dim),
            cache_modifier=".cg",
        )  # [head_dim, BLOCK_K]
        qk = (
            tl.dot(my_q, cur_k, out_dtype=tl.float32) * softmax_scale
        )  # [BLOCK_Q, BLOCK_K]
        cur_k = None
        cur_v = tl.load(
            v_ptrs + k_block_start * (num_kv_heads * head_dim),
            cache_modifier=".cg",
        )  # [BLOCK_K, head_dim]
    ...
    for k_block_start in range(
        my_q_block * BLOCK_Q, (my_q_block + 1) * BLOCK_Q, BLOCK_K
    ):
        k_block_start = tl.multiple_of(k_block_start, BLOCK_K)
        cur_k = tl.load(
            k_ptrs + k_block_start * (num_kv_heads * head_dim),
        ...
        qk = tl.where(
            ...
            & (
                range_my_q[:, None]
                >= (k_block_start + tl.arange(0, BLOCK_K))[None, :]
            ),
            qk,
            float("-1e20"),
        )
