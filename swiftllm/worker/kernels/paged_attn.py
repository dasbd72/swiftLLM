...
def _fwd_paged_attention_phase1(
    ...
    k_ptrs = (
        k_cache
        + my_kv_head_id * block_size * head_dim
    ...
    if my_start_token_idx + seq_block_size > my_seq_len:
        ...
        for block_i in range(0, my_num_blocks):
            ...
            k_block = tl.load(
                k_ptrs + block_index * num_kv_heads * block_size * head_dim
            )  # [block_size, head_dim]
            attn_score = tl.sum(
            ...
            v_block = tl.load(
                v_ptrs + block_index * num_kv_heads * block_size * head_dim
            )  # [block_size, head_dim]
...
def paged_attention(
    ...
    if k_cache.device.type == "cuda":
        ...
        swiftllm_c.cpu_paged_attention(
            q,
            k_cache,
            v_cache,
            block_table,
