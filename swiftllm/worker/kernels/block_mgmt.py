...
def _fwd_set_block_table_and_num_seq_alloc_blocks_kernel(
    ...
    my_candidate_block_start_index = (
        tl.load(block_needed_cumsum + my_batch_id) - my_block_needed
    )
    my_num_allocated_blocks = tl.load(num_seq_allocated_blocks + my_seq_id)
    for i in range(my_block_needed):
        my_block_id = tl.load(
            candidate_blocks + my_candidate_block_start_index + i
        )
        tl.store(
            block_table
            + my_seq_id * max_blocks_per_seq
            + my_num_allocated_blocks
            + i,
            my_block_id,
        )
    tl.store(
        num_seq_allocated_blocks + my_seq_id,
        my_num_allocated_blocks + my_block_needed,
    )
...
def _fwd_gather_allocated_blocks_and_unset_kernel(
    ...
    my_seq_id = tl.load(seq_ids + my_batch_id)
    my_num_blocks = tl.load(num_seq_allocated_blocks + my_seq_id)
    my_num_allocated_blocks_cumsum = tl.load(
        num_allocated_blocks_cumsum + my_batch_id - 1,
        mask=my_batch_id > 0,
        other=0,
    )
    for i in range(my_num_blocks):
        my_block_id = tl.load(block_table + my_seq_id * max_blocks_per_seq + i)
        tl.store(
            gathered_block_ids + my_num_allocated_blocks_cumsum + i,
            my_block_id,
        )
        tl.store(is_block_free + my_block_id, True)
    tl.store(num_seq_allocated_blocks + my_seq_id, 0)
...
def gather_allocated_blocks_and_unset(
    ...
    gathered_block_ids = torch.empty(
        (num_allocated_blocks_cumsum[-1].item(),),
        dtype=torch.int32,
