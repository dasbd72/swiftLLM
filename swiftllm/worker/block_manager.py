...
from .kernels.block_mgmt import gather_allocated_blocks_and_unset
from .kernels.block_mgmt import set_block_table_and_num_seq_alloc_blocks
from .kernels.block_mgmt import unset_block_table_and_num_seq_alloc_blocks
...
class BlockManager:
    ...
    def __init__(
        ...
        self.num_seq_allocated_blocks = torch.zeros(
            (max_seqs_in_block_table,), dtype=torch.int32, device="cuda"
        )
        ...
        self.is_block_free = torch.ones(
            (num_blocks,), dtype=torch.bool, device="cuda"
        )
    ...
    def allocate_blocks_for_seqs(
        self, seq_ids: torch.Tensor, target_lens: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        """
        target_num_blocks = (
            target_lens + (self.block_size - 1)
        ) // self.block_size
        assert (
            self.num_seq_allocated_blocks[seq_ids] <= target_num_blocks
        ).all(), f"""(On {self.device_name}) Logic error: Some sequences have more blocks already allocated than needed.
    ...
    def free_blocks_for_seqs(self, seq_ids: torch.Tensor):
        ...
        """
        self.num_free_blocks += torch.sum(
            self.num_seq_allocated_blocks[seq_ids]
        ).item()
    ...
    def gather_allocated_blocks_and_free(
        self, seq_ids: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        """
        gathered_block_ids = gather_allocated_blocks_and_unset(
            self.num_seq_allocated_blocks,
            self.block_table,
            seq_ids,
            self.is_block_free,
