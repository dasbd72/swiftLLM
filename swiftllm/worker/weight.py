...
class WeightBase:
    ...
    def load_weights(self, getter: callable):
        ...
        for item in self.registered_weights:
            ...
            ), f"getter() returned None for {item.key} ({item})"
            assert isinstance(
...
class LlamaTransformerLayerWeight(WeightBase):
    ...
    def _post_process_after_load(self, getter: callable):
        ...
        self.up_gate_proj = torch.cat(
            (self.up_proj, self.gate_proj), dim=0
        ).contiguous()
    ...
    def __init__(
        ...
        for i in range(self.model_config.num_layers):
            layer = LlamaTransformerLayerWeight(
                i, self.model_config, self.dtype, self.model_version
            )
...
def load_weights(
    ...
    if model_version == "auto":
        ...
        if os.path.exists(config_path):
            ...
            if "rope_scaling" in config_data and isinstance(
                config_data["rope_scaling"], dict
            ):
    ...
    if use_dummy:
        ...
        if len(safetensor_files) > 0:
            ...
            def weight_getter_real(item: RegisteredWeightItem):
                file_name = (
                    safetensor_index[item.key]
                    if safetensor_index is not None
                    else safetensor_filename
                )
            ...
            # Use PyTorch
            pytorch_index_path = os.path.join(
            ...
            def weight_getter_real(item: RegisteredWeightItem):
                file_name = (
                    pytorch_index[item.key]
                    if pytorch_index is not None
                    else pytorch_filename
                )
                file_path = os.path.join(model_path, file_name)
