...
// The source_block_ids and target_block_ids are the block ids of the blocks to
// be swapped. source_block_ids[0] will be copied to target_block_ids[0] and so
// on `is_swap_in` defines whether the swap is a swap-in or swap-out (swap-in
// means to swap from CPU to GPU, swap-out means to swap from GPU to CPU)
//
...
//
// Future work: Now the number of cudaMemcpyAsync calls is equal to 2x the
// number of blocks to swap. We can reduce the number of cudaMemcpyAsync calls
// by grouping nearby blocks together and perform a single invocation
void swap_blocks(const std::vector<int64_t> &source_block_ids,
  ...
  while (next_index < num_blocks_to_swap) {
    ...
    if (is_swap_in) {
      ...
      cudaMemcpyAsync((char *)k_cache.data_ptr() +
                          start_target_block_id * block_size_in_bytes,
                      (char *)k_swap.data_ptr() +
                          start_source_block_id * block_size_in_bytes,
                      cur_segment_size_in_bytes, cudaMemcpyHostToDevice,
                      stream);
      cudaMemcpyAsync((char *)v_cache.data_ptr() +
                          start_target_block_id * block_size_in_bytes,
                      (char *)v_swap.data_ptr() +
                          start_source_block_id * block_size_in_bytes,
                      cur_segment_size_in_bytes, cudaMemcpyHostToDevice,
                      stream);
    } else {
      // Copy from GPU to CPU
      cudaMemcpyAsync((char *)k_swap.data_ptr() +
                          start_target_block_id * block_size_in_bytes,
                      (char *)k_cache.data_ptr() +
                          start_source_block_id * block_size_in_bytes,
                      cur_segment_size_in_bytes, cudaMemcpyDeviceToHost,
                      stream);
      cudaMemcpyAsync((char *)v_swap.data_ptr() +
                          start_target_block_id * block_size_in_bytes,
                      (char *)v_cache.data_ptr() +
                          start_source_block_id * block_size_in_bytes,
                      cur_segment_size_in_bytes, cudaMemcpyDeviceToHost,
                      stream);
    }
